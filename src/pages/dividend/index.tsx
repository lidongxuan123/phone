...
import { filterData, moneyAll, monthData } from "../../utils/filterData";
...
const [seriesList, setSeriesList] = useState<any>()
    ...
    let [recordXaxis, setRecordXaxis] = useState<any>([])
    let [initXaxis, setInitXaxis] = useState<any>([])
    const lineColor = ['#FFe119', '#e6194B', '#4363d8', '#ffffff', '#42d4f4']
    let timer = useRef()
    const handleData = (data: any) => {
        let result = monthData(data.reverse())
        return result
    }
    // 获取股票名称
    ...
    const handleSubmit = async () => {
        ...
        for (let i = 0; i < 4; i++) {
            if (month + i > 12) {
                initXaxisTemp.push(`${year + 1}-${(month + i) % 12}`)
            } else {
                initXaxisTemp.push(`${year}-${(month + i) % 12}`)
            }
        }
        setInitXaxis(initXaxisTemp)
        // 获取输入的数据
        ...
        xAxis: {
            ...
            axisLabel: {
                ...
                formatter: function (value: any) {
                    ...
                    let xpoint = year + '年' + (month < 10 ? '0' + month : month) + '月'
                    let xpointTemp = year + '-' + (month < 10 ? month : month)
                    if (recordTemp.indexOf(xpointTemp) == -1) {
                        recordTemp.push(xpointTemp)
                        return xpoint; // 格式为 "yyyy-MM"
                    }
                },
                // interval: 10,  // 显示所有标签
            },
